...
class Post(db.Model):
    ...
    #imageSrc = db.Column(db.String(140))
...
def submit():
    ...
    if not form_data:
        flash('You have not selected any ingredients. Select some!')
    ...
    if form_data:
        ...
        response = unirest.get(url,
          headers={
            "X-Mashape-Key": "RXdnl2hcErmshmDAHAFQoy5DkZZjp1NsfWejsnDe2BCiLPi79E",
        ...
        save1='<h3><span>' + title1 + '</span></h3>' + '<a target="_blank" href="' + link1 + '"><img class="img-rounded recipepic" src="' + image1 + '"></a>'
        save2='<h3><span>' + title2 + '</span></h3>' + '<a target="_blank" href="' + link2 + '"><img class="img-rounded recipepic" src="' + image2 + '"></a>'
        save3='<h3><span>' + title3 + '</span></h3>' + '<a target="_blank" href="' + link3 + '"><img class="img-rounded recipepic" src="' + image3 + '"></a>'
        save4='<h3><span>' + title4 + '</span></h3>' + '<a target="_blank" href="' + link4 + '"><img class="img-rounded recipepic" src="' + image4 + '"></a>'
        ...
        return render_template("recipes.html", ingredients=ingredients, 
            ...
            abc=link1, defi=image1, ghi=title1)
    return redirect(url_for('index'))
...
def save1():
    if g.user is not None and g.user.is_authenticated:
        ...
        #title = formdata['abc']
        #link = formdata['defi']
        #imageurl = formdata['ghi']
        #print title
        # rec1 = 'Here is a new recipe:"' + recipe1 + '"Saved from search'  
        #print rec1
        post = Post(body=recipe1, timestamp=datetime.utcnow(),author=g.user)
        db.session.add(post)
        db.session.commit()
